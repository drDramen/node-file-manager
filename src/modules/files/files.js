...
export const createFolder = async (args) => {
  const { currentDir } = stateManager.getState();
  const [folderName] = args.split(' ');
  const folderPath = getAbsolutePath(currentDir, folderName);

  const isFolderExist = await targetExists(folderPath);

  if (isFolderExist) {
    throw new InputException(`${ERROR_MSG}: Folder already exists`);
  }
...
export const createFile = async (args) => {
  ...
  if (isFileExist) {
    throw new InputException(`${ERROR_MSG}: File already exists`);
  }
